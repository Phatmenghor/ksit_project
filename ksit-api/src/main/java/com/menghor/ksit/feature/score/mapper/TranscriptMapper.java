...
public interface TranscriptMapper {
    ...
    @Mapping(target = "placeOfBirth", source = "placeOfBirth")
    @Mapping(target = "degree", source = "classes.degree")
    @Mapping(target = "dateOfAdmission", ignore = true)
    @Mapping(target = "dateOfGraduation", ignore = true)
    ...
    // NEW: Set default values - these will be overridden in service
    ...
    @Mapping(target = "scoreSessionId", ignore = true)
    ...
    // REMOVED: The old mapScoreToTranscript method since we're handling this manually in service
    // This method was causing issues with the calculations
