...
import com.menghor.ksit.feature.menu.service.impl.MenuServiceImpl;
...
public class MenuController {
    ...
    public ApiResponse<String> migrateAllUsersToNewPermissions() {
        ...
        try {
            ...
            if (menuService instanceof MenuServiceImpl menuServiceImpl) {
                menuServiceImpl.updateAllUsersToNewPermissions();
                return ApiResponse.success("Permission migration completed successfully",
                        "All users have been updated to the new permission system");
            } else {
                return ApiResponse.success("Migration not available",
                        "Permission migration method not available in current implementation");
            }
        } catch (Exception e) {
            log.error("Error during permission migration: {}", e.getMessage(), e);
            return ApiResponse.success("Migration failed",
