...
public class UserSpecification {
    ...
    public static Specification<UserEntity> searchByName(String searchTerm) {
        return (root, query, criteriaBuilder) -> {
            ...
            // Full name searches - Khmer (First + Last)
            Expression<String> khmerFullName = criteriaBuilder.concat(
                    criteriaBuilder.coalesce(root.get("khmerFirstName"), ""),
                    criteriaBuilder.concat(" ", criteriaBuilder.coalesce(root.get("khmerLastName"), ""))
            );
            predicates.add(criteriaBuilder.like(criteriaBuilder.lower(khmerFullName), term));

            // Full name searches - Khmer (Last + First) - for different name order
            Expression<String> khmerFullNameReversed = criteriaBuilder.concat(
                    criteriaBuilder.coalesce(root.get("khmerLastName"), ""),
                    criteriaBuilder.concat(" ", criteriaBuilder.coalesce(root.get("khmerFirstName"), ""))
            );
            predicates.add(criteriaBuilder.like(criteriaBuilder.lower(khmerFullNameReversed), term));

            // Full name searches - English (First + Last)
            Expression<String> englishFullName = criteriaBuilder.concat(
                    criteriaBuilder.coalesce(root.get("englishFirstName"), ""),
                    criteriaBuilder.concat(" ", criteriaBuilder.coalesce(root.get("englishLastName"), ""))
            );
            predicates.add(criteriaBuilder.like(criteriaBuilder.lower(englishFullName), term));

            // Full name searches - English (Last + First) - for different name order
            Expression<String> englishFullNameReversed = criteriaBuilder.concat(
                    criteriaBuilder.coalesce(root.get("englishLastName"), ""),
                    criteriaBuilder.concat(" ", criteriaBuilder.coalesce(root.get("englishFirstName"), ""))
            );
            predicates.add(criteriaBuilder.like(criteriaBuilder.lower(englishFullNameReversed), term));
