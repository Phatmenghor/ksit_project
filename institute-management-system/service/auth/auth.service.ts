...
import { clearUserId, storeUserId } from "@/utils/local-storage/user-info/userId";
import { clearUsername, storeUsername } from "@/utils/local-storage/user-info/username";
...
export const AdminChangePasswordService = async (data: ChangePasswordByAdminModel) => {
  try {
    const response = await axiosClientWithAuth.post(`/v1/auth/change-password-by-admin`, data);
    return response.data;
  } catch (error) {
    if (axios.isAxiosError(error)) {
      throw new Error(error.response?.data?.message || "Change password failed. Please try again.");
    }
    throw new Error("An unexpected error occurred. Please try again.");
  }
};
...
export const ChangePasswordService = async (data: ChangePasswordModel) => {
  try {
    const response = await axiosClientWithAuth.post(`/v1/auth/change-password`, data);
    return response.data;
  } catch (error) {
    if (axios.isAxiosError(error)) {
      throw new Error(error.response?.data?.message || "Change password failed. Please try again.");
