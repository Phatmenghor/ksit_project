  ...
  return (
    <Card className="border border-gray-100 shadow-sm">
      <CardContent className="py-3 px-4 space-y-3">
        ...
        {/* Search (fixed width) + filters (min-width, fill remaining) + clear icon */}
        <div className="flex flex-wrap items-end gap-2">

          {/* Search: fixed 260px, never grows */}
          ...
          {/* Filters: each has min-width, grows to fill remaining space */}
          {essentialFilters.map((filter) => (
            <div key={filter.id} className="flex-1 min-w-[148px]">
              {renderFilter(filter)}
            </div>
          ))}

          {/* Clear icon button: same h-9, no text */}
          {anyFilterActive && config.onClearAll && (
            <button
              type="button"
              onClick={config.onClearAll}
              title="Clear all filters"
              className="h-9 w-9 flex items-center justify-center rounded-md border border-input bg-background text-muted-foreground hover:bg-red-50 hover:border-red-300 hover:text-red-500 transition-colors flex-shrink-0"
            >
              <X className="h-4 w-4" />
            </button>
          )}
