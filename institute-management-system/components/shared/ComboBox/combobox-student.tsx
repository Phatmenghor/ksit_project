...
interface ComboboxSelectedProps {
  ...
  onChangeSelected: (item: StudentModel) => void; // Callback to notify parent about the selection change
  disabled?: boolean;
}
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          ...
          {dataSelect ? dataSelect.username : "Select a student..."}
              ...
              {data?.map((item, index) => (
                ...
                >
                  ...
                  <span className="truncate max-w-[85%]">
                    {`${item.username} - ${item.englishFirstName} ${item.englishLastName}`}
                  </span>
                </CommandItem>
              ))}
            </CommandGroup>
