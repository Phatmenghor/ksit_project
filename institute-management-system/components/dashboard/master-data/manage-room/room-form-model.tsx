...
export type RoomFormData = z.infer<typeof roomFormSchema> & { id?: number };
...
export function RoomModal({ isOpen, onClose, onSubmit, initialData, mode, isSubmitting = false }: RoomModalProps) {
  ...
  const handleSubmit = async (data: RoomFormData) => {
    setIsUploading(true);
    try {
      const submitData: RoomFormData = { ...data, status: Constants.ACTIVE };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="w-full max-w-lg p-0 flex flex-col max-h-[90vh]">
        <FormHeader
          title={isCreate ? "Add Room" : "Edit Room"}
          description={`Fill in the information below to ${isCreate ? "create" : "update"} a room.`}
            ...
            <FormBody>
              <FormField control={form.control} name="name" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input placeholder="Enter room name" {...field} autoFocus /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
            </FormBody>
